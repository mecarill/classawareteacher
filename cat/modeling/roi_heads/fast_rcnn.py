...
def fast_rcnn_inference_single_image(
    ...
    scores_keep = scores_keep[filter_inds[:,0],:]

    result = Instances(image_shape)
    result.pred_boxes = Boxes(boxes)
    result.scores = scores
    result.full_scores = scores_keep
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.loss_reg_val = loss_reg_val
        self.dia_loss = dia_loss
    ...
    def from_config(cls, cfg, input_shape):
        return {
            ...
            "loss_reg_val"          : cfg.LOSS_REG_VAL,
            "dia_loss"              : cfg.DIA_LOSS
            # fmt: on
        }
    ...
    def losses(self, predictions, proposals,class_info=None):
        """
        Args:
        ...
        gt_oht_classes = (
            cat([p.gt_oht_classes for p in proposals], dim=0) if len(proposals) else torch.empty(0)
        )
        box_losses_w = (
            cat([p.box_loss for p in proposals], dim=0) if len(proposals) else torch.empty(0)
        )
        ...
        # Get predicted classes not including background
        #pred_classes = torch.max(scores[:,:-1],dim=1).indices

        #TODO: Test1 Do one with removing background and another without
        pred_classes = torch.max(scores,dim=1).indices

        w = torch.ones_like(gt_classes)*1.0
        if class_info is not None:
            ci = class_info.clone()
            mix_mask = torch.sum(gt_oht_classes,dim=1)!=1
            #mix_gt_oht_classes = gt_oht_classes[mix_mask]
            ...
            w = (w+self.loss_reg_val)/(1+self.loss_reg_val)
    ...
    def box_reg_loss(self, proposal_boxes, gt_boxes, pred_deltas, gt_classes,box_losses_w):
        """
        Args:
        ...
        box_losses_w=box_losses_w.type(torch.bool)[fg_inds]
        fg_inds = fg_inds[box_losses_w]
